...
const createDayPlan = async (payload) => {
    const MODPlan = MODModel[payload.collection].findOne({
        week: payload.week,
    });
    if (MODPlan)
        return new ApiError(
            httpStatus.CONFLICT,
            'Duplicate: Plan already created'
        );
        ...
        for (let mealIndex = 0; mealIndex < plan.meals.length; mealIndex++) {
            ...
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                ...
                items[itemIndex] = {
                    ...items[itemIndex],
                    ...menu,
                    food: `${menu.food} (${menu.conversion})`,
                };
            }
        }
    }
...
const getPlanById = async ({ id, collection }) => {
    ...
    for (let planIndex = 0; planIndex < MODPlan.plans.length; planIndex++) {
        ...
        for (let mealIndex = 0; mealIndex < plan.meals.length; mealIndex++) {
            const items = plan.meals[mealIndex].items;
            for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
                let menu = await FoodModel.findOne({
                    _id: items[itemIndex].id,
                }).lean();
