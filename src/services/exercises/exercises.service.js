...
const updateExercise = async (payload) =>{
    const exercise = await ExerciseModel.findByIdAndUpdate(payload.id, {...payload});
    return exercise;
}

const getExercises = async () =>{
    const exercises = await ExerciseModel.find({}).exec();
    if(!exercises || exercises.length === 0) throw new ApiError(httpStatus.NOT_FOUND, 'Exercises not found');
    return exercises;
}
